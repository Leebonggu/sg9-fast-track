...
- **recipientSeq는 행 대조 키로 쓰면 안 된다** — 무명(자리표시) 행 39개가 실소유자 행의
  recipientSeq를 **공유**한다 (예: 901-206 정순현과 무명 902-503이 같은 seq=332809, 3중복도 있음).
  seq로 Map을 만들면 중복 키 오매칭으로 "명부 41행이 재배정됐다"는 허상이 보인다
  (2026-08-22 실제로 겪음 — 명부 내용은 우리 46건 외 변동 0건이었다). **행 대조는 동-호+이름으로만**
