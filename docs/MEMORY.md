...
- **`npm run econsent-writeback -- <업체_신속통합.xlsx>`** (`web/scripts/make-econsent-writeback.mts`)
  → 원본 옆에 `_서면동의반영.xlsx` 사본 생성 + `docs/raw/`에 명단 md (개인정보 — 커밋 금지)
- 대상: 종이 신통동의 + 신분증 제출(업로드>0 또는 종이수령) 세대. `제출상태`만 '서면동의(직접)'으로.
