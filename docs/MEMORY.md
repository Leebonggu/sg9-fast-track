...
- 2026-08-22 입안(plan) 모드 실행·등록 완료: 대상 53세대 → 표기 44세대 46행 → 업체 일괄등록
  46건 전원 성공(제출일시 18:40) → 재다운로드 검증 전원 서면동의(직접)+자동완비(서면 1→47행),
  선택 항목은 '-'로 남음(예측 적중) → 정방향 임포트로 루프 마감: 입안 완전 500→545 세대
  (+45 = 우리 44세대 + 신규 전자동의 1건 917-1210). 917-1004는 신분증 없어 기준 미달 제외.
  입안 `선택 항목`(추진위/직접조합)은 벌크로 입력 불가 — 서면동의 행은 '-'로 남는다.
  선례: 업체가 8/3 수동 등록한 908-107 김길순B도 선택 항목 '-' + 자동완비 (입안 파일 실측).
- **업체 명부는 살아있는 문서** — 2026-08-22 등록 전후 30분 사이에 업체가 무명(자리표시) 행
  41개를 실소유자로 재배정(39명 이름 채움, 그중 4명은 이미 전자동의: 908-805·908-1501·914-403·915-105).
  재다운로드 파일은 recipientSeq도 재사용/이동되므로 **행 대조는 동-호+이름으로만**. 임포트는
  반드시 같은 시각에 내려받은 신통·입안 쌍으로 (한쪽만 새로 받으면 파서가 쌍 불일치로 거부).
