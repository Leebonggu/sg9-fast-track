/**
 * 상계주공 9단지 신속통합기획 사전 전자동의 시스템 (v3)
 *
 * v2 대비 변경:
 * - 실거주 여부 선택 추가 (실거주 / 비거주)
 * - 비거주 → 주민등록상주소 입력 + 종이 동의서 PDF 다운로드 안내
 * - 실거주 → 주소 입력 없이 바로 동의 + 방문 수거 안내
 *
 * 사용법:
...
// ============================================================
var CONFIG = {
  FORM_TITLE: '상계주공 9단지 아파트 신속통합기획 추진 사전 전자동의서 v3',
  SHEET_TITLE: '상계주공 9단지 사전 전자동의 관리 v3',
  BUILDINGS: [
  ...
  CONSENT_FOLDER_URL: 'https://drive.google.com/drive/folders/1WWCqVRde3wLdrYYclEG_ePbVJVeUwnd5',
  // 동별 시트 컬럼 헤더
  BUILDING_HEADERS: [
    '타임스탬프', '성명', '연락처', '호수',
    '실거주여부', '주민등록상주소', '사전동의여부', '개인정보동의여부',
...
// ============================================================
function setupAll() {
  Logger.log('=== 상계주공 9단지 사전 전자동의 시스템 셋업 시작 ===');
...
function createForm_() {
  ...
  form.setConfirmationMessage(
    ...
    '[ 비거주자 안내 ]\n' +
    '비거주자는 아래 링크에서 종이 동의서 양식과 작성방법을 확인해주세요.\n\n' +
    '📎 동의서 양식 및 작성방법:\n' +
    CONFIG.CONSENT_FOLDER_URL + '\n\n' +
    '감사합니다.'
  );
  ...
  residenceItem.setTitle('실거주 여부')
    .setHelpText('해당 동/호수에 현재 거주하고 계신가요?')
    .setRequired(true);

  // ── 실거주자 경로: 동의 → 제출 안내 ──

  var residentConsentPage = form.addPageBreakItem();
  residentConsentPage.setTitle('사전동의 및 개인정보 수집 동의');

  var consentItem1 = form.addCheckboxItem();
  consentItem1.setTitle('신속통합기획 추진 검토를 위한 사전동의')
    .setRequired(true);
  consentItem1.setChoiceValues(['신속통합기획 추진 검토에 동의합니다.']);

  var privacyItem1 = form.addCheckboxItem();
  privacyItem1.setTitle('개인정보 수집 및 이용 동의')
    .setRequired(true);
  privacyItem1.setChoiceValues(['개인정보 수집 및 이용에 동의합니다.']);

  var residentEndPage = form.addPageBreakItem();
  residentEndPage.setTitle('제출 안내');
  residentEndPage.setHelpText(
    '입력하신 연락처로 확인 연락이 갈 수 있으니 참고 부탁드립니다.\n' +
    '담당자가 방문하여 정식 서면 동의서를 안내드릴 예정입니다.\n\n' +
    '입력하신 정보가 정확한지 확인 후 제출해 주세요.'
  );
  residentEndPage.setGoToPage(FormApp.PageNavigationType.SUBMIT);

  // ── 비거주자 경로: 주소 입력 → 동의 → 제출 안내 + 다운로드 ──

  var addressPage = form.addPageBreakItem();
  addressPage.setTitle('주민등록상주소 입력');
  addressPage.setHelpText('비거주(임대 등)이신 경우, 현재 주민등록상 주소를 입력해주세요.');
  ...
  var nonResidentConsentPage = form.addPageBreakItem();
  nonResidentConsentPage.setTitle('사전동의 및 개인정보 수집 동의');

  var consentItem2 = form.addCheckboxItem();
  consentItem2.setTitle('신속통합기획 추진 검토를 위한 사전동의')
    .setRequired(true);
  consentItem2.setChoiceValues(['신속통합기획 추진 검토에 동의합니다.']);

  var privacyItem2 = form.addCheckboxItem();
  privacyItem2.setTitle('개인정보 수집 및 이용 동의')
    .setRequired(true);
  privacyItem2.setChoiceValues(['개인정보 수집 및 이용에 동의합니다.']);

  var nonResidentEndPage = form.addPageBreakItem();
  nonResidentEndPage.setTitle('제출 안내');
  nonResidentEndPage.setHelpText(
    '입력하신 연락처로 확인 연락이 갈 수 있으니 참고 부탁드립니다.\n\n' +
    '아래 링크에서 종이 동의서 양식과 작성방법을 확인해주시기 바랍니다.\n\n' +
    '📎 동의서 양식 및 작성방법:\n' +
    CONFIG.CONSENT_FOLDER_URL + '\n\n' +
    '입력하신 정보가 정확한지 확인 후 제출해 주세요.'
  );
  nonResidentEndPage.setGoToPage(FormApp.PageNavigationType.SUBMIT);

  // ── 페이지 분기 설정 ──

  residenceItem.setChoices([
    residenceItem.createChoice('실거주', residentConsentPage),
    residenceItem.createChoice('비거주 (임대 등)', addressPage)
  ]);

  // 주소 입력 후 → 비거주자 동의 페이지로 이동
  addressPage.setGoToPage(nonResidentConsentPage);
...
function createBuildingSheets_(ss) {
  CONFIG.BUILDINGS.forEach(function(building) {
    ...
    sheet.setColumnWidth(5, 100);  // 실거주여부
    sheet.setColumnWidth(6, 250);  // 주민등록상주소
    sheet.setColumnWidth(7, 120);  // 사전동의여부
    sheet.setColumnWidth(8, 130);  // 개인정보동의여부
    sheet.setColumnWidth(9, 80);   // 입력경로
    sheet.setColumnWidth(10, 120); // 동의서수거여부
    sheet.setColumnWidth(11, 100); // 수거일
    sheet.setColumnWidth(12, 80);  // 수거자
...
//    e.values 순서 (페이지 분기 — 경로별로 빈값 위치가 다름):
//    Timestamp, 성명, 연락처, 소유동, 호수, 실거주여부,
//    사전동의1(실거주), 개인정보1(실거주),
//    주민등록상주소(비거주), 사전동의2(비거주), 개인정보2(비거주)
// ============================================================
function onFormSubmit(e) {
  try {
    ...
    var values = e.values;
    Logger.log('e.values: ' + JSON.stringify(values));
    ...
    var residence = String(values[5] || '');

    // 실거주자: 인덱스 6,7에 동의값, 8은 빈값
    // 비거주자: 인덱스 6,7은 빈값, 8에 주소, 9,10에 동의값
    var consent = String(values[6] || values[9] || '');
    var privacyConsent = String(values[7] || values[10] || '');
    var address = String(values[8] || '');

    Logger.log('응답: ' + name + ' / ' + building + ' / ' + unit + '호 / 실거주: ' + residence + ' / 동의: ' + consent);
    ...
    // 이메일 알림 발송
    sendNotification_(name, building, unit, residence, consent, timestamp, duplicateInfo.isDuplicate);
...
function debugAndSync() {
  ...
  for (var r = 2; r <= lastRow; r++) {
    ...
    var phone = data['연락처'] || '';
    var unit = data['호수'] || '';
    var residence = data['실거주 여부'] || '';
    var address = data['주민등록상주소'] || '';
    var consent = data['신속통합기획 추진 검토를 위한 사전동의에 참여하시겠습니까?'] || '';
    var privacyConsent = data['개인정보 수집 및 이용 동의'] || '';

    var newRow = [
      timestamp, name, phone, unit, residence, address, consent, privacyConsent,
      '온라인', false, '', '', ''
    ];
...
// 이메일 알림 발송
// ============================================================
function sendNotification_(name, building, unit, residence, consent, timestamp, isDuplicate) {
