...
export default function ConsentPage() {
  ...
  async function loadDashboard(pushHistory = true) {
    ...
    try {
      ...
      setScreen('main');
      if (pushHistory) {
        window.history.pushState({ screen: 'main' }, '', '/consent');
      }
    } catch {
      alert('데이터 로딩 실패');
    }
  ...
  async function loadBuilding(building: string, pushHistory = true) {
    ...
    try {
      ...
      setScreen('grid');
      if (pushHistory) {
        window.history.pushState({ screen: 'grid', building }, '', `/consent?b=${encodeURIComponent(building)}`);
      }
    } catch {
      alert('데이터 로딩 실패');
    }
  ...
  function downloadCSV() {
    ...
    a.download = `${building}_신통기획접수_${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
    ...
    mainContent = (
      <div className="min-h-screen bg-gray-50 pb-16">
        ...
        <header className="bg-[#2F5496] text-white p-3.5 flex items-center sticky top-0 z-40">
          ...
          <button onClick={() => { setScreen('dashboard'); window.history.pushState({ screen: 'dashboard' }, '', '/consent?v=dashboard'); }} className="bg-white/20 px-3 py-1.5 rounded-lg text-sm">현황</button>
        </header>
  ...
  } else if (screen === 'grid' && buildingData) {
  ...
  } else if (screen === 'dashboard' && dashboard) {
    mainContent = (
      <div className="min-h-screen bg-gray-50 pb-16">
        {loadingOverlay}
        <header className="bg-[#2F5496] text-white p-3.5 flex items-center sticky top-0 z-40">
